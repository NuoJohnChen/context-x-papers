...
This repository houses the core intelligence for **semantic literature retrieval** and **topic discovery**. It serves as the backend engine for the Researcher Module within the **XtraMCP** framework [1], powering the **PaperDebugger** writing assistant [3]. It also provides the technological infrastructure required to operationalize the **Community-Federated Conference (CFC)** model, which addresses the sustainability crisis in centralized AI conferences [2].
