...
def split_into_topics(text: str, max_topics: int = 6) -> list[str]:
    ...
    if bullet_lines:
        ...
        if len(line_hits) >= 2:
            ...
            # If still a single segment, consider 'and/以及/和' when it appears at least twice
            if len(non_empty) <= 1:
...
def recommend():
    ...
    if recommender is None:
        return jsonify({"error": "推荐器未初始化"}), 500
    ...
    try:
        ...
        if not workshop_description:
            return jsonify({"error": "请提供workshop描述"}), 400
        
        # 规范化日期输入（YYYY-MM-DD 或 YYYY-MM）
...
def health():
    ...
    global recommender
    if recommender is None:
        return jsonify({"status": "error", "message": "推荐器未初始化"})
    return jsonify({"status": "ok", "message": "推荐器运行正常"})
    ...
    try:
        ...
        try:
            ...
            if n_topics < 2 or n_topics > 50:
                return jsonify({"error": "主题数量必须在2-50之间"}), 400
        except Exception:
            return jsonify({"error": "无效的主题数量"}), 400
...
def feedback():
    """接收用户对推荐结果的反馈（点赞/踩），以 JSONL 追加存储"""
    try:
...
def list_organizations():
    ...
    if recommender is None or not hasattr(recommender, 'df'):
        return jsonify({"error": "推荐器未初始化或无数据"}), 500
    try:
...
def export_excel():
    ...
    try:
        ...
        if not recommendations:
            return jsonify({"error": "没有推荐结果可导出"}), 400
...
def export_pdf():
    ...
    try:
        ...
        if not papers:
            return jsonify({"error": "没有论文可导出"}), 400
...
def paper2poster_proxy(path):
    """代理paper2poster请求到paper2poster_app"""
    from flask import request as flask_request
    
    # 直接调用paper2poster_app的相应处理函数
    if path == '':
...
def api_paper2poster_upload():
    """代理API请求到paper2poster_app"""
    # 直接复制请求数据并调用paper2poster_app的函数
    try:
        # 检查是否有文件上传
