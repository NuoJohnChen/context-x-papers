...
def main():
    ...
    # Count total IDs and IDs starting with 25 (this year)
    print("Counting papers (from Kaggle metadata)...")
    total_papers = 0
    total_25_prefix = 0
    papers_25_prefix = []  # Store 25-prefix papers for downloading
    
    for aid in iter_arxiv_ids_from_kaggle(meta_path):
        total_papers += 1
        if normalize_arxiv_id(aid).startswith("25"):
            total_25_prefix += 1
            papers_25_prefix.append(aid)

    print(f"Found {total_papers} papers with IDs")
    print(f"Found {total_25_prefix} papers with IDs starting with '25' (current year)")
    
    # Download PDFs (only 25-prefix papers)
    downloaded = 0
    skipped = 0
    failed = 0
    ...
    print(f"\nStarting {'async' if args.async_mode else 'parallel'} download of {total_25_prefix} papers with '25' prefix...")
    print(f"Using {args.workers} {'concurrent connections' if args.async_mode else 'parallel workers'}")
    
    # Prepare download tasks
    download_tasks = []
    for arxiv_id in papers_25_prefix:
