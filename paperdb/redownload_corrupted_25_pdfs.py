#!/usr/bin/env python3
"""
Redownload corrupted PDFs for a given year prefix (e.g., 24 for 2024) using arXiv API.
...
3. Uses arXiv bulk API to redownload and replace corrupted files

Examples:
  python3 redownload_corrupted_25_pdfs.py --year 24 --progress
  python3 redownload_corrupted_25_pdfs.py --year 2020 --dry-run  # Identify only
"""
...
ARXIV_BULK_API_BASE = "https://export.arxiv.org/api/query"
#BATCH_SIZE = 20  # arXiv API batch size
MAX_WORKERS = 10  # Concurrent downloads
CORRUPTED_SIZE = 1853  # Size in bytes that indicates corruption
# Default User-Agent (can be overridden via --user-agent). Include a way to contact you per arXiv's policy
DEFAULT_USER_AGENT = "arxiv-redownload-script/1.0 (+https://github.com/nuojohnchen)"
...
def download_pdf_via_api(arxiv_id: str, output_path: Path, progress: bool = False, headers: dict | None = None) -> bool:
    """Download PDF using arXiv bulk API."""
    try:
        # Rate limiting: wait 0.1 seconds before each request
        time.sleep(0.5)
        
        # arXiv bulk API endpoint - use proper XML query format
        url = f"{ARXIV_BULK_API_BASE}?id_list={arxiv_id}&start=0&max_results=1"
        
        # First get metadata to verify paper exists
        api_headers = {"Accept": "application/atom+xml"}
        if headers:
            api_headers.update(headers)
        response = requests.get(url, timeout=30, headers=api_headers)
        response.raise_for_status()
        
        # Check if paper exists in XML response - look for entry tag with correct ID (with or without version)
        import re
        response_text = response.text
        # Match: <id>http://arxiv.org/abs/2401.00012v1</id> or <id>http://arxiv.org/abs/2401.00012v2</id>
        id_pattern = re.compile(rf'<id>http://arxiv\.org/abs/({re.escape(arxiv_id)}v\d+)</id>')
        match = id_pattern.search(response_text)
        if not match:
            if progress:
                print(f"Paper {arxiv_id} not found in arXiv API response")
            return False

        arxiv_id_with_version = match.group(1)
        # Now download the actual PDF using the matched versioned arxiv_id
        pdf_url = f"https://arxiv.org/pdf/{arxiv_id_with_version}.pdf"
        pdf_headers = {"Accept": "application/pdf"}
        if headers:
            pdf_headers.update(headers)
        pdf_response = requests.get(pdf_url, timeout=60, stream=True, headers=pdf_headers)
        pdf_response.raise_for_status()
        
        # Check if we got a real PDF (not error page)
        content_type = pdf_response.headers.get('content-type', '')
        if 'application/pdf' not in content_type:
            if progress:
                print(f"Warning: {arxiv_id} returned non-PDF content: {content_type}")
...
def redownload_corrupted_pdfs(corrupted_ids: Set[str], pdfs_dir: Path, 
                             max_workers: int, progress: bool, dry_run: bool, headers: dict | None) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for arxiv_id in corrupted_ids:
            ...
            future = executor.submit(download_pdf_via_api, arxiv_id, output_path, progress, headers)
...
def main():
    parser = argparse.ArgumentParser(description="Redownload corrupted PDFs for a given year prefix using arXiv API")
    ...
    parser.add_argument("--user-agent", default=DEFAULT_USER_AGENT, help="Custom User-Agent to use for arXiv requests")
    ...
    common_headers = {"User-Agent": args.user_agent}
    ...
    results = redownload_corrupted_pdfs(
        corrupted_ids, pdfs_dir, args.max_workers, args.progress, args.dry_run, common_headers
    )
